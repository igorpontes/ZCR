...
<%@ Register Src="tab_requerimento.ascx" TagName="tab_requerimento" TagPrefix="uc1" %>
<%@ Register Src="tab_titulacoes.ascx" TagName="tab_titulacoes" TagPrefix="uc2" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    ...
    <script type="text/javascript">
        ...
        /*function ActiveTabChanged(sender, e) {
            var CurrentTab = $get('ctl00_SampleContent_CurrentTab');
        ...
        // do what ever i want with lastTab value
            ...
            //try
            //{
             //var index = $find("<%= TabContainer1.ClientID  %>").get_activeTabIndex();
            
            if (indexTab == 0) {
                $get("<%= btnDisparaUCPessoais.ClientID %>").click();
            }else if (indexTab == 1) {
                $get("<%= btnDisparaUCTitulacao.ClientID %>").click();
            } else if (indexTab == 2) {
                $get("<%= btnDisparaUCPortaria.ClientID %>").click();
            } else if (indexTab == 3) {
                $get("<%= btnDisparaUCCI.ClientID %>").click();
            }
            else if (indexTab == 4) {
                $get("<%= btnDisparaUCAviso.ClientID %>").click();
            }
            else if (indexTab == 5) {
                $get("<%= btnDisparaUCRequerimento.ClientID %>").click();
            }
            else if (indexTab == 5) {
                $get("<%= btnDisparaUCOutros.ClientID %>").click();
            }
            //}catch(Exception exz){
    ...
    public void tabChanged()
        {
            string abaAtiva = TabContainer1.ActiveTab.HeaderText;
                                        ...
                                        <asp:UpdatePanel ID="UpdatePanelTabContainer" runat="server">
                                            <ContentTemplate>
                                                <asp:TabContainer ID="TabContainer1" runat="server" ActiveTabIndex="0" Width="700px"
                                                    Height="150px" AutoPostBack="true" OnClientActiveTabChanged="getTab" 
                                                    ...
                                                    <asp:TabPanel ID="TabPanelTitulacao" runat="server" HeaderText="Titulações">
                                                        <ContentTemplate>
                                                            <asp:Button ID="btnDisparaUCTitulacao" Style="display: none" runat="server" 
                                                                onclick="btnDisparaUCTitulacao_Click"  />
                                                            <uc3:tab_default ID="tab_default2" runat="server" />
                                                        </ContentTemplate>
                                                    </asp:TabPanel>
                                                    <asp:TabPanel ID="TabPanelPortaria" runat="server" HeaderText="Portarias">
                                                        <ContentTemplate>
                                                            <asp:Button ID="btnDisparaUCPortaria" Style="display: none" runat="server" 
                                                                onclick="btnDisparaUCPortaria_Click" />
                                                            <uc3:tab_default ID="tab_default3" runat="server" />
                                                        </ContentTemplate>
                                                    </asp:TabPanel>
                                                    <asp:TabPanel ID="TabPanelCI" runat="server" HeaderText="CIs">
                                                        <ContentTemplate>
                                                            <asp:Button ID="btnDisparaUCCI" Style="display: none" runat="server" 
                                                                onclick="btnDisparaUCCI_Click"  />
                                                            <uc3:tab_default ID="tab_default4" runat="server" />
                                                        </ContentTemplate>
                                                    </asp:TabPanel>
                                                    <asp:TabPanel ID="TabPanelAviso" runat="server" HeaderText="Aviso de Férias">
                                                        <ContentTemplate>
                                                            <asp:Button ID="btnDisparaUCAviso" Style="display: none" runat="server" 
                                                                onclick="btnDisparaUCAviso_Click" />
                                                            <uc3:tab_default ID="tab_default5" runat="server" />
                                                        </ContentTemplate>
                                                    </asp:TabPanel>
                                                    <asp:TabPanel ID="TabPanelRequerimento" runat="server" HeaderText="Requerimentos">
                                                        <ContentTemplate>
                                                            <asp:Button ID="btnDisparaUCRequerimento" Style="display: none" runat="server" 
                                                                onclick="btnDisparaUCRequerimento_Click"  />
                                                            <uc3:tab_default ID="tab_default6" runat="server" />
                                                        </ContentTemplate>
                                                    </asp:TabPanel>
                                                    <asp:TabPanel ID="TabPanelOutros" runat="server" HeaderText="Outros">
                                                        <ContentTemplate>
                                                            <asp:Button ID="btnDisparaUCOutros" Style="display: none" runat="server" 
                                                                onclick="btnDisparaUCOutros_Click"  />
                                                            <uc3:tab_default ID="tab_default7" runat="server" />
                                                        </ContentTemplate>
                                                    </asp:TabPanel>
                                                </asp:TabContainer>
                                            </ContentTemplate>
                                ...
                                <tr>
                                    <td align="center" colspan="3">
                                        <asp:ImageButton ID="ImageButtonCadastrar" runat="server" ImageUrl="~/imagens/botao_Enviar.png"
                                            TabIndex="25" onclick="ImageButtonCadastrar_Click" />
                                    </td>
                                </tr>
                            </table>
                        </div>
